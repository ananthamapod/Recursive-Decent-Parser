...
static int variable()
{
	...
	if(!is_identifier(token)) {
		ERROR("Invalid variable identifier %s\n",token);
		exit(EXIT_FAILURE);
	}
	curr = (int)token;
	next_token();
...
static int expr()
{
	...
	case 'd':
		reg = next_register();
		CodeGen(LOAD, reg, variable(), EMPTY_FIELD);
		return reg;
	case '0':
	case '1':
	case '2':
...
static void assign()
{
	/* YOUR CODE GOES HERE */
	int from_reg;
	int to_token = variable();
	if(token != '=') {
		ERROR("Incorrect assign statement\n");
		exit(EXIT_FAILURE);
	}
	next_token();
	from_reg = expr();
	CodeGen(STORE, to_token, from_reg, EMPTY_FIELD);
}
...
static void swap()
{
	...
	int id1, id2;
	if(token != '%') {
		ERROR("Incorrect syntax for swapping variable values\n");
		exit(EXIT_FAILURE);
	}
	next_token();
	reg1 = next_register();
	id1 = variable();
	CodeGen(LOAD, reg1, id1, EMPTY_FIELD);
	reg2 = next_register();
	id2 = variable();
	CodeGen(LOAD, reg2, id2, EMPTY_FIELD);
	CodeGen(STORE, id1, reg2, EMPTY_FIELD);
	CodeGen(STORE, id2, reg1, EMPTY_FIELD);
}
...
static void morestmts()
{
	...
	if(token != ';') {
		ERROR("Expected \';\' divider between statements\n");
		exit(EXIT_FAILURE);
	}
	next_token();
	stmtlist();
